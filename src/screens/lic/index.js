...
import { routerLic } from "../../navigation/routers/profileRouter";
...
function MiniDrawer({ modules, activeProfileModule, setActiveModule,userFullName,menuOpen,setMenuOpen }) {
  ...
  React.useEffect(() => {
    ...
    if(modules){
      let parent = modules.filter((i) => i.active == 1 && i.parent !=null&& i.menupath==39 && i.path== 
  ...
  const menuItems = modules
    .filter((i) => i.active == 1 && i.parent == null  && i.menupath==39)
    .map((item, index) => {
      return (
        <>
          {item.path ? (
