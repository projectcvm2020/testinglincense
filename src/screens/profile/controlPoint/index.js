...
import ListF from "../../../forms/controlPoint/list";
import Detail from "../../../forms/controlPoint/detail";
import { useLocation ,useHistory} from "react-router-dom";
import CircularProgress from "@material-ui/core/CircularProgress";
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
function controlPoint({ token }) {
  ...
  const getItems = () => {
    ...
    if(search)
        s="search="+search
    getRequest("personresource/getcontrolPoint?"+s+"&offSet="+offSet, token)
      .then((res) => {
        if (res.data) {
          if(items.length>0)
...
export default connect(mapStateToProps)(controlPoint);
