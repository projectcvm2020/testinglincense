...
function detail({ token, userId,recordId2 ,setShowModal }) {
  ...
  useEffect(() => {
    if (recordId) {
      setLoading(true);
      getRequest("personresource/getcontrolpoint?id=" + recordId, token)
        .then((res) => {
          setLoading(false);
  ...
  const submit = () => {
    setSaving(true);
    postRequest(
      "personresource/setcontrolpoint",
      {
        data: initState,
        id: recordId,
      },
      ...
      .then((res) => {
        ...
        if (!res.data.controlPointDberror) toast.success(res.data.message);
        else toast.error(res.data.controlPointDberror.message);
        setSaving(false);
