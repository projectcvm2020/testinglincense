...
import ListF from "../../../forms/positions/list";
import Detail from "../../../forms/positions/detail";
import { useLocation ,useHistory} from "react-router-dom";
import CircularProgress from "@material-ui/core/CircularProgress";
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
function positions({ token }) {
  ...
  const getItems = (load=true) => {
    ...
    getRequest("personresource/getposition?"+s+limit, token)
      .then((res) => {
        if (res.data) {
  ...
  return (
    <div>
      <h3>Cargos de la CVM</h3>

      {items ? (
        <List
          ListForm={(props) => <ListF {...props} />}
          title="Cargos de la CVM"
          srcRedir="positions"
          idFieldName="id"
          list={items}
