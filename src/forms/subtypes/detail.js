...
import { getGetActityType } from "../../components/loadSelect";
...
function detail({ token,userId,recordId2 ,setShowModal }) {
  ...
  useEffect(() => {
    if (recordId) {
      setLoading(true);
      getRequest("personresource/getsubtypeslist?id=" + recordId, token)
        .then((res) => {
          setLoading(false);
  ...
  const submit = () => {
    ...
    setSaving(true);
    postRequest(
      "personresource/setactivitytypes",
      {
        data: initState,
        id: recordId,
      },
      ...
      .then((res) => {
        ...
        if (!res.data.activitytypeDberror) toast.success(res.data.message);
        else toast.error(res.data.activitytypeDberror.message);
        setSaving(false);
        ...
        <div className={classes.content}>
          ...
          <div>
            <TextField
              ...
              error={validateForm("", initState.parentid, null, null)}
              helperText={
                validating && initState.parentid == "" ? "Campo requerido" : ""
              }
              select
              required
              label="Tipo de actividad"
              value={initState.parentid}
              onChange={(t) => {
                setInitState(
                  Object.assign({}, initState, { parentid: t.target.value })
                );
              }}
              style={{width:"50%"}}
            >
