import React, { useState, useEffect } from "react";
import { connect } from "react-redux";
import { getRequest } from "../../../../utility/net/urls";
import { View } from "react-native-web";
import { useParams } from "react-router-dom";
import Grid from "@material-ui/core/Grid";
import { makeStyles } from "@material-ui/core/styles";
import Divider from "@material-ui/core/Divider";
import Moment from "moment";
import CircularProgress from "@material-ui/core/CircularProgress";
const useStyles = makeStyles((theme) => ({
  divcenter: {
    justifyContent: "center",
    alignItems: "center",
    display: "flex",
  },
}));
function printBild({ token }) {
  const classes = useStyles();

  const params = useParams();
  const [item, setItem] = useState(null);
  const [photo1, setPhoto1] = useState(null);
  const [photo2, setPhoto2] = useState(null);
  useEffect(() => {
    getRequest("affiliateaccount/getrecord?id=" + params.recordId, token)
      .then((res) => {
        console.log(res);
        setItem(res.data.record);
      })
      .catch((err) => {
        console.log("Algo pasó al intentar conectar con el servidor.", err);
      });
  }, []);
  useEffect(() => {
    if (item) {
      getRequest("img/getimg?img=" + item.photomin, token)
        .then((res) => {
         
          setPhoto1(res.img);
        })
        .catch((err) => {
          console.log("Algo pasó al intentar conectar con el servidor.", err);
        });
      getRequest("img/getimg?img=" + item.photouser, token)
        .then((res) => {
          console.log("result photo", res);
          setPhoto2(res.img);
        })
        .catch((err) => {
          console.log("Algo pasó al intentar conectar con el servidor.", err);
      });
    }
  }, [item]);

  return (
    <>
      {item&&<div
        style={{
          width: 612,
          textAlign: "justify",
          fontSize: 12,
          paddingLeft: 30,
          paddingRight: 30,
          zoom: params.zoom + "%",
        }}
        id="continer"
      >
        <img
          style={{
            width: 612,
          }}
          src={require("../../../../assets/img/bannercertificatecvm.jpg")}
        />
        <h5>Recepción de oro</h5>
        <Divider />
        <b>Datos personales</b>
        <Grid container>
          <Grid item xs={4}>
            <span>Nombre:</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.name}</span>
          </Grid>
          <Grid item xs={4}>
            <span>C.I.:</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.dni}</span>
          </Grid>
          <Grid item xs={4}>
            <span>Direccion:</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.entitydirection}</span>
          </Grid>
          <Grid item xs={4}>
            <span>Municipio:</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.parish}</span>
          </Grid>
          <Grid item xs={4}>
            <span>Sector:</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.sector}</span>
          </Grid>
          <Grid item xs={4}>
            <span>Teléfono:</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.entityphonenumber}</span>
          </Grid>
          <Grid item xs={4}>
            <span>Código de carnet:</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.licence}</span>
          </Grid>
          <Grid item xs={4}>
            <span>Fecha de recepción:</span>
          </Grid>
          <Grid item xs={8}>
            <span>{Moment(item.data).format("DD/MM/YYYY")}</span>
          </Grid>
          <Grid item xs={4}>
            <span>Funcionario:</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.nameuser}</span>
          </Grid>
        </Grid>
        <Divider />
        <b>Oro recibido</b>

        <Grid container>
          <Grid item xs={4}>
            <span>Mineral comercializado</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.validkilograms}</span>
          </Grid>
          <Grid item xs={4}>
            <span>Tipo de ley</span>
          </Grid>
          <Grid item xs={8}>
            <span>Gramiado</span>
          </Grid>
          <Grid item xs={4}>
            <span>Peso bruto</span>
          </Grid>
          <Grid item xs={8}>
            <span>{item.credit}</span>
          </Grid>
         {/*<Grid item xs={4}>
            <span>Peso fino:</span>
          </Grid>
          <Grid item xs={8}>
            <span>3.3</span>
        </Grid>*/} 
        </Grid>
        <Divider />
        <b>Memoria fotográfica</b>
        <Grid container>
          <Grid item xs={4}>
            <div className={classes.divcenter}>Foto del afiliado</div>
            <div className={classes.divcenter}>
              <View>
                <View
                  style={{
                    width: 150,
                    height: 150,
                    marginTop: 10,
                    zIndex: 100,
                    alignItems: "center",
                  }}
                >
                  {photo2 ? (
                    <>
                      <div
                        style={{
                          height: 150,
                          width: 150,
                          backgroundImage:
                            "url(data:image/png;base64," + photo2 + ") ",
                          backgroundPosition: "center",
                          backgroundSize: "cover",
                        }}
                      />
                    </>
                  ) : (
                    <CircularProgress
                      color="#000"
                      size={15}
                      style={{ marginTop: 70 }}
                    />
                  )}
                </View>
              </View>
            </div>
          </Grid>
          <Grid item xs={4}>
            <div className={classes.divcenter}>Huella digital</div>
          </Grid>
          <Grid item xs={4}>
            <div className={classes.divcenter}>Foto del mineral</div>
            <div className={classes.divcenter}>
              <View>
                <View
                  style={{
                    width: 150,
                    height: 150,
                    marginTop: 10,
                    zIndex: 100,
                    alignItems: "center",
                  }}
                >
                  {photo1 ? (
                    <>
                      <div
                        style={{
                          height: 150,
                          width: 150,
                          backgroundImage:"url(data:image/png;base64," + photo1 + ") ",
                          backgroundPosition: "center",
                          backgroundSize: "cover",
                        }}
                      />
                    </>
                  ) : (
                    <CircularProgress
                      color="#000"
                      size={15}
                      style={{ marginTop: 70 }}
                    />
                  )}
                </View>
              </View>
            </div>
          </Grid>
        </Grid>
        <Divider />
        <View style={{ flexDirection: "row" ,flex:1,width:612,marginTop:50}}>
            <View style={{textAlign:"center",flex:0.4}}>
            <div style={{height:1,backgroundColor:"#000"}}></div>
            <div >Firma del funcionario</div>
            </View>
            <View style={{textAlign:"center",flex:0.4,marginLeft:10}}>
            <div style={{height:1,backgroundColor:"#000"}}></div>
            <div>Firma del afiliado</div>

            </View>
          </View>              
          
      </div>}
    </>
  );
}
const mapState = ({ authStorage }) => {
  return {
    token: authStorage.token,
  };
};
export default connect(mapState)(printBild);
