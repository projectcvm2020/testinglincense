...
function detail({ token, userId,recordId2 ,setShowModal,getItems }) {
  ...
  useEffect(() => {
    if (recordId) {
      setLoading(true);
      getRequest("personresource/getfuel?id=" + recordId, token)
        .then((res) => {
          setLoading(false);
  ...
  const submit = () => {
    ...
    postRequest(
      ...
      .then((res) => {
        ...
        if (!res.data.fuelDberror) {
          getItems(false)
          toast.success(res.data.message);
        }
        else toast.error(res.data.fuelDberror.message);
        setSaving(false);
      ...
      <div className={classes.content}>
          ...
          <TextField
            ...
            error={
              ...
              validateForm("", initState.taxe, null, null)
            }
            helperText={
              initState.taxe!="" &&
              validating 
