...
function detail({ token, userId,recordId2 ,setShowModal }) {
  ...
  useEffect(() => {
    if (recordId) {
      setLoading(true);
      getRequest("personresource/getparishlist?id=" + recordId, token)
        .then((res) => {
          setLoading(false);
  ...
  const submit = () => {
    ...
    postRequest(
      ...
      .then((res) => {
        ...
        if (!res.data.parishDberror) toast.success(res.data.message);
        else toast.error(res.data.parishDberror.message);
