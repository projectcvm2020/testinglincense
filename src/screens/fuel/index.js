...
import { routerFuel } from "../../navigation/routers/profileRouter";
...
function MiniDrawer({ modules, activeProfileModule, setActiveModule,userFullName,menuOpen,setMenuOpen }) {
  ...
  const activetRoute = (route) => {
    return window.location.href.split("/")[5]  ==route ? true : false;
  };
 
  React.useEffect(() => {
    if(modules){
        let parent = modules.filter((i) => i.active == 1 && i.parent !=null&& i.menupath==35 && i.path== 
  ...
  const menuItems = modules
    .filter((i) => i.active == 1 && i.parent == null  && i.menupath==35)
    .map((item, index) => {
      return (
        <>
          {item.path ? (
