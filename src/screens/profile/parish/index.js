...
import ListF from "../../../forms/parish/list";
import Detail from "../../../forms/parish/detail";
import { useLocation ,useHistory} from "react-router-dom";
import CircularProgress from "@material-ui/core/CircularProgress";
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import QRCode from "react-qr-code";
function parish({ token }) {
  ...
  const getItems = () => {
    ...
    if(search)
        s="search="+search
    getRequest("personresource/getparishlist?"+s+"&offSet="+offSet, token)
      .then((res) => {
        if (res.data) {
          if(items.length>0)
...
export default connect(mapStateToProps)(parish);
