...
import ListF from "../../../forms/mineralTypes/list";
import Detail from "../../../forms/mineralTypes/detail";
import { useLocation ,useHistory} from "react-router-dom";
import CircularProgress from "@material-ui/core/CircularProgress";
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
function mineralTypes({ token }) {
  ...
  const getItems = () => {
    ...
    if(search)
        s="search="+search
    getRequest("personresource/getmineraltypes?"+s+"&offSet="+offSet, token)
      .then((res) => {
        if (res.data) {
          if(items.length>0)
...
export default connect(mapStateToProps)(mineralTypes);
